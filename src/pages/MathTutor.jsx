        ...
        <div className="container my-5 p-5 text-center shadow-lg rounded-5">
            <div className="row">
                <div className="col">
                    ...
                    <p className="lead text-danger">DISCLAIMER: This is an AI that needs time to think, so responses will be delayed. Please be patient for responses.</p>
                ...
                <form className="">
                    {messages.map((message, index) => (
                        <input type="text" readOnly={true} value={message.content} key={index}
                               className={`form-control ${message.role === "user" ? "bg-primary-subtle" : "bg-success-subtle"} my-2`}/>
                    ))}
                </form>
            </div>
            <div className="row">
                <form className="input-group" onSubmit={async (event) => {
                    ...
                    <input type="text" value={input} className="form-control bg-primary-subtle"
                           placeholder="Type something here..." onChange={(event) => {
